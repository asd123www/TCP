...
int queryMacAddress(const struct in_addr dest, const void *buffer, int broadcast) {
    ...
    exit(0);
    // how to send ARP address?
    ((u_char *)buffer)[0] = 0x00;
    ((u_char *)buffer)[1] = 0x0c;
    ((u_char *)buffer)[2] = 0x29;
    ((u_char *)buffer)[3] = 0x21;
    ((u_char *)buffer)[4] = 0xe7;
    ((u_char *)buffer)[5] = 0x8b;
    return 0;
}
...
int sendIPPacket(const struct in_addr src, const struct in_addr dest, int proto, const void *buf, int len, int broadcast, int device) {
    if (len > 65516) {
        printf("too many bytes in IP packet!\n");
        return -1;
    }
    ...
    if (destmac == NULL || buffer == NULL) {
        printf("Can't malloc the space needed!\n");
        return -1;
    }
...
int ipCallbackExample(const void* buf, int len) {
